...
Archived
    ...
    impl Default for AlertRuleStatus {
...
pub enabled: bool,
    ...
    if self.query.trim().is_empty() { return Err(ValidationError("query must not be empty".into())); }
    if !(0.0..=1.0).contains(&self.threshold) { return Err(ValidationError("threshold must be between 0 and 1".into())); }
    if self.delivery_channel.trim().is_empty() { return Err(ValidationError("delivery_channel must not be empty".into())); }
            Ok(())
        }
